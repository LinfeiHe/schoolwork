...
image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x),
                                          data_transforms[x])
                  for x in ['train', 'val']}
batch_size = {'train': 24, 'val': 4}
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size[x], shuffle=True, num_workers=4)
               for x in ['train', 'val']}
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i, data in enumerate(dataloaders[phase]):
                ...
                #     break
                # if phase == 'val' and i % 20 == 19:
                #     print('valing test complete')
                #     break

                if phase == 'train' and i % 200 == 199:
                    print('{} iter: {:.4f} Loss_path: {:.4f}'.format(phase, i+1, running_loss_path))
                    running_loss_path = 0.0

                if phase == 'val' and i % 1600 == 1599:
                    print('{} iter: {:.4f} Loss_path: {:.4f}'.format(phase, i + 1, running_loss_path))
                    running_loss_path = 0.0
    ...
    torch.save(best_model_wts, 'best_model_wts_nasnet.pkl')
    torch.save(best_acc, 'best_acc_nasnet.pkl')
    return model
...
model_ft.linear = nn.Linear(num_ftrs, 80)
...
"""
train Loss: 0.0422 Acc1: 0.7163 Acc3: 0.8752
val iter: 1600.0000 Loss_path: 1136.2879
val Loss: 0.1784 Acc1: 0.7987 Acc3: 0.9395

Training complete in 1483m 52s
Best val Acc: 0.940590
"""
