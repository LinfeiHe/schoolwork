...
for k = 1 : 20
    ...
    net = newrbe(train_in', train_out', k);
...
text(best_sigma,best_test_accuracy-0.1, ['best accuracy( ', num2str(best_sigma), ' , ', num2str(best_test_accuracy),' )'])
title('train and test accuracy with different sigma by RBF');
xlabel('不同sigma值');
ylabel('预测准确率');
